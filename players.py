...
def whatcaniplay(deck,name,deckhandlens):
    ...
    # if('FVR' in mapping and deckhandlens[int(name)^1]): possible += ['FVR']*len(mapping['FVR']) 
    # if('STF' in mapping): possible += ['STF']*len(mapping['STF']) 
    # if('C1' in mapping and deckhandlens[int(name)^1]): possible += ['C1']*(len(mapping['C1'])//2)
    # if('C2' in mapping and deckhandlens[int(name)^1]): possible += ['C2']*(len(mapping['C2'])//2)
    # if('C3' in mapping and deckhandlens[int(name)^1]): possible += ['C3']*(len(mapping['C3'])//2)
    # if('C4' in mapping and deckhandlens[int(name)^1]): possible += ['C4']*(len(mapping['C4'])//2)
    # if('C5' in mapping and deckhandlens[int(name)^1]): possible += ['C5']*(len(mapping['C5'])//2)
    return possible
